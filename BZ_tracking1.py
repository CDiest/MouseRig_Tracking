...
import numpy as np #For handling numerical data and saving csv
import time #For performance timing
import datetime #For timestamps and file naming
import os #Directory and file management
import PySpin #camera control and acquisition
import imageio #saving video files
from settings import * #Custom configuation settings
import signal #For handling termination signals for graceful shutdown
...
def tracking(Exp_ID, rig_pos, t_pos, filechunk,record=False, resize = False):
    ...
    while datetime.datetime.now() < end_time:
        ...
        (_, contours, _) = cv2.findContours(image_thresholded.copy(), cv2.RETR_EXTERNAL,
